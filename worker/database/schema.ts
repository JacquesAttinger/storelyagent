...
/**
 * Stripe Connect Accounts table - Stores connected Stripe Express accounts for payment processing
 */
...
/**
 * User Domains table - Stores domains purchased by users and linked to their stores
 */
export const userDomains = sqliteTable('user_domains', {
    id: text('id').primaryKey(),
    userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
    appId: text('app_id').references(() => apps.id, { onDelete: 'set null' }), // Linked store
    ...
    userDomainIdx: uniqueIndex('user_domains_user_domain_idx').on(table.userId, table.domain),
}));
