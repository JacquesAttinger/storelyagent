...
export function setupAuthRoutes(app: Hono<AppEnv>): void {
    ...
    // Access token retrieval (for cross-origin store API calls from admin dashboard)
    authRouter.get('/access-token', setAuthLevel(AuthConfig.authenticated), adaptController(AuthController, AuthController.getAccessToken));
